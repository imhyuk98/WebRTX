...
pub fn process(
    ...
    // SAFETY: Some shaders (e.g., malformed or edge-case intersection shaders) might not
    // trigger visit_function_definition (name mismatch or early return) leaving
    // referenced_gl_global_variables = None and causing a panic below.
    if analyzer.referenced_gl_global_variables.is_none() {
        analyzer.referenced_gl_global_variables = Some(HashMap::new());
    }
    ...
    Ok(ProcessedShaderInfo {
        ...
        packing_code: if shader_stage == "rint" {
            if let Some(ref hit_attrs) = analyzer.hit_attributes {
        ...
        global_variables: analyzer
            .referenced_gl_global_variables
            .as_ref()
            .map(|m| m.iter().map(|var| *var.0 as u32).collect())
