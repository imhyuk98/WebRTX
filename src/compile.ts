...
export async function aggregateAndCompileShaders(device: GPUDevice, descriptor: GPURayTracingPipelineDescriptor, tlas: GPURayTracingAccelerationContainer_top): Promise<[GPUShaderModule, number]> {
  ...
  // Access extension stages lazily to avoid TDZ before patch() initializes globals
  const Stage = (globalThis as any).GPUShaderStageRTX;
  if (!Stage) {
    throw new Error('GPUShaderStageRTX not initialized');
  }

  const canonicalShaderStageEntryName = (stg: number, i: number): string => {
    let prefix = '';
    switch (stg) {
      case Stage.RAY_GENERATION:
        prefix = '_crt_user_rgen_'; break;
      case Stage.RAY_ANY_HIT:
        prefix = '_crt_user_rahit_'; break;
      case Stage.RAY_CLOSEST_HIT:
        prefix = '_crt_user_rchit_'; break;
      case Stage.RAY_MISS:
        prefix = '_crt_user_rmiss_'; break;
      case Stage.RAY_INTERSECTION:
        prefix = '_crt_user_rint_'; break;
      default:
  ...
  // Build maps at runtime now that Stage is initialized
  const USER_FUNCTIONS_TABLE_NAME: Record<number, string> = {
    [Stage.RAY_GENERATION]: '_CRT_USER_RGEN_TABLE',
    [Stage.RAY_MISS]: '_CRT_USER_RMISS_TABLE',
    [Stage.RAY_INTERSECTION]: '_CRT_USER_RINT_TABLE',
    [Stage.RAY_CLOSEST_HIT]: '_CRT_USER_RCHIT_TABLE',
    [Stage.RAY_ANY_HIT]: '_CRT_USER_RAHIT_TABLE',
  };
  const SHADER_STAGE_NAME: Record<number, 'rgen'|'rmiss'|'rint'|'rchit'|'rahit'> = {
    [Stage.RAY_GENERATION]: 'rgen',
    [Stage.RAY_MISS]: 'rmiss',
    [Stage.RAY_INTERSECTION]: 'rint',
    [Stage.RAY_CLOSEST_HIT]: 'rchit',
    [Stage.RAY_ANY_HIT]: 'rahit',
  } as any;
  ...
  const stages: Map<number, number[]> = new Map();
  ...
  // TODO: impl
  const [bvhGeometriesDescs, bvhReferencedGeoBuffers] = (tlas as any).getBvhGeometryBuffersAndDescriptors() as [Array<{ vBufferIndex:number; iBufferIndex:number; vboOffset:number; vboStride:number; vioOffset:number; vioStride:number; owningGeometryType_todo_deprecate:number; owningGeometryFlags:number; }>, Map<GPUBuffer, number>];
  const numGeomBuffers = bvhReferencedGeoBuffers.size;
  console.log('[WebRTX] compiler numGeomBuffers =', numGeomBuffers);
...
#define _CRT_USER_GEO_BUFFERS_ACCESSOR_CASES(wordIndex) _GET_FROM_BUFFER_CASE_x${numGeomBuffers}(wordIndex)
  ...
  const userFunctionsTable = Object.values(Stage).map((stg: any) => {
  ...
  let spirv: any;
  try {
    spirv = glslang.compileGLSL(completeGLSL, 'compute', false);
  } catch (e) {
    console.error('GLSL compilation failed. Dumping completeGLSL for debugging...');
    console.error(completeGLSL);
    throw e;
  }
