...
import { _assert, _debugAssert, debugWarn } from './util';
...
async function loadBvhWasmModule(): Promise<BvhWasmModule> {
  ...
  // Initialize the wasm module (wasm-pack default export)
  if (typeof (_wasm_bvh as any).default === 'function') {
    await (_wasm_bvh as any).default();
  }
  patchStagingBuffer(_wasm_bvh);
  return _wasm_bvh;
}

// Eagerly initialize in the background
loadBvhWasmModule().catch(() => {});

export async function initBvhWasm(): Promise<void> {
  await loadBvhWasmModule();
}

export interface BuiltAabbBlasNodes {
  nodeCount: number;
  nodeData: Uint8Array;
}
...
export class Tlas {
  ...
  console.log('[WebRTX] total geometry buffers (unique):', buffers.size);
